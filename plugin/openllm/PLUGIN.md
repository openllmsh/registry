...
plugin version removes its hooks too, without the script knowing their
...
```
curl -fsSL "$GATEWAY_ORIGIN/api/plugins/openllm/install.sh?mode=uninstall" | bash
```
...
only when the region was all it held), and deletes the plugin dir. The
