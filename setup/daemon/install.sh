#!/usr/bin/env bash
# openllm-self-sha256: 3229d6cc3ef6f9913ced08474efd459ac6dfe6780d284f806bc77d5555fe9528
...
record_install_stamp() {
  ...
  (umask 077; printf '%s' "1.6.39-registry.8" > "$tmp" && mv -f "$tmp" "$dir/setup-daemon.version") 2>/dev/null || { rm -f "$tmp"; true; }
  ...
  if [[ "${OPENLLM_REGISTRY_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]]; then
    tmp="$dir/.setup-daemon.commit.$$"
    (umask 077; printf '%s' "$OPENLLM_REGISTRY_COMMIT" > "$tmp" && mv -f "$tmp" "$dir/setup-daemon.commit") 2>/dev/null || { rm -f "$tmp"; true; }
  fi
}
clear_install_stamp() {
...
read_install_commit() {
  local value
  value="$(cat "$HOME/.openllm/installed/setup-daemon.commit" 2>/dev/null || true)"
