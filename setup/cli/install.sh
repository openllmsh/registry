#!/usr/bin/env bash
# openllm-self-sha256: ca931a016a6b6ee049d39d79fdb66829f9daa15cf17380dccd1c1afdf036df81
...
record_install_stamp() {
  ...
  (umask 077; printf '%s' "1.6.39-registry.9" > "$tmp" && mv -f "$tmp" "$dir/setup-cli.version") 2>/dev/null || { rm -f "$tmp"; true; }
  ...
  if [[ "${OPENLLM_REGISTRY_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]]; then
    tmp="$dir/.setup-cli.commit.$$"
    (umask 077; printf '%s' "$OPENLLM_REGISTRY_COMMIT" > "$tmp" && mv -f "$tmp" "$dir/setup-cli.commit") 2>/dev/null || { rm -f "$tmp"; true; }
  fi
}
clear_install_stamp() {
...
read_install_commit() {
  local value
  value="$(cat "$HOME/.openllm/installed/setup-cli.commit" 2>/dev/null || true)"
