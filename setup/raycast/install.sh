#!/usr/bin/env bash
# openllm-self-sha256: ecd13ee1492b2de442776bd21e268800d5166af908219e0ad29b6b40428e02c1
...
record_install_stamp() {
  ...
  (umask 077; printf '%s' "1.6.39-registry.8" > "$tmp" && mv -f "$tmp" "$dir/setup-raycast.version") 2>/dev/null || { rm -f "$tmp"; true; }
  ...
  if [[ "${OPENLLM_REGISTRY_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]]; then
    tmp="$dir/.setup-raycast.commit.$$"
    (umask 077; printf '%s' "$OPENLLM_REGISTRY_COMMIT" > "$tmp" && mv -f "$tmp" "$dir/setup-raycast.commit") 2>/dev/null || { rm -f "$tmp"; true; }
  fi
}
clear_install_stamp() {
...
read_install_commit() {
  local value
  value="$(cat "$HOME/.openllm/installed/setup-raycast.commit" 2>/dev/null || true)"
