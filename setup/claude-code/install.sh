#!/usr/bin/env bash
# openllm-self-sha256: 59986302865a55590df6fb67503ad4d22315bab2008b50809f7cd6cae6b94485
...
record_install_stamp() {
  ...
  (umask 077; printf '%s' "1.6.39-registry.8" > "$tmp" && mv -f "$tmp" "$dir/setup-claude-code.version") 2>/dev/null || { rm -f "$tmp"; true; }
  # The registry artifact commit this install came from (delivered via env by
  # the daemon / gateway preamble) — lets the dashboard link the DEVICE's
  # installed version to its exact public commit. Best-effort, display-only.
  if [[ "${OPENLLM_REGISTRY_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]]; then
    tmp="$dir/.setup-claude-code.commit.$$"
    (umask 077; printf '%s' "$OPENLLM_REGISTRY_COMMIT" > "$tmp" && mv -f "$tmp" "$dir/setup-claude-code.commit") 2>/dev/null || { rm -f "$tmp"; true; }
  fi
}
clear_install_stamp() {
...
read_install_commit() {
  local value
  value="$(cat "$HOME/.openllm/installed/setup-claude-code.commit" 2>/dev/null || true)"
